...
\State $\textit{maps} \gets \text{saliency maps w.r.t. each class using a zero input}$
\State $\textit{all outliers} \gets \text{empty list}$
\State $\textit{all preds} \gets \text{empty list}$
\State $\textit{recovered mask} \gets \text{empty array of size } w \times h \times c$
...
\For{$\textit{i} \gets \text{in } w \times h \times c$}
    \If {$\text{pixel} \textit{ i} \text{is outlier in at least } \frac{K}{2}+1\;\; maps$}
        \State $\textit{recovered mask}[i] \gets \text{mark as outlier}$
    \EndIf
\EndFor

\For{$\textit{i} \gets \text{in } 1,...,\textit{trials}$}
    \State $\textit{input} \gets \text{generate a random input using the \textit{recovered mask}}$
    \State $\textit{patched} \gets \text{patch every image in \textit{sample} using \textit{input}}$
...
\State $\textit{objective class} \gets \text{mode of \textit{flipped}}$
\State $\textit{proportion} \gets \text{proportion of \textit{flipped} equal to \textit{objective class}}$

\Return \textit{objective class}, \textit{proportion}
...
The outlier detection was performed using an EllipticEnvelope.\footnote{We used scikit-learn \textit{EllipticEnvelope} implementation}
